...
var icon_size = 60;
	$('.flatty_shadow').each(function() {		
		var bg_color = $(this).attr("data-bg-color") || '#F5F5F5';
		var shadow_color = $(this).attr("data-shadow-color") || '#df3618';
		var icon_color = $(this).attr("data-icon-color") || '#ffffff';
		var angle = $(this).attr("data-angle") || 50;
		var depth = $(this).attr("data-depth") || 190;
		...
		var icon_pos_y = $(this).attr("data-pos-y") || -15;
		bg_size = $(this).attr("data-bg-size") || 120;
		...
		shadow_color = '  rgba(238, 238, 238, 1)';		
		icon_color = ' rgba(255, 255, 255, 1)';
			...
			$(this).children( '.flatty_icon' ).css('background', bg_color);
		    ...
		    $(this).children('.flatty_icon').find('.centered').css({ marginLeft : (parseInt(icon_pos_x)*2.3)-55+'px', marginTop : (parseInt(icon_pos_y)*1.5)-60+'px' });
	...
	var icon_pos =parseInt(icon_size.toString().split('px')[0]) * -1;		    
